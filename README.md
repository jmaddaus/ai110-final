...
End-to-end video walkthrough (~2 minutes) showing the system running on three classic-rock and blues seeds: Highway to Hell by AC/DC (modern AC/DC-style band Airbourne surfaces at #1 and #2), Voodoo Child by Hendrix (blues-heritage matches with Gary Moore, Eric Clapton with B.B. King, and Rory Gallagher), and Black Dog by Led Zeppelin in default mode versus Discovery mode (canonical neighbors versus non-obvious picks).

- **Loom link:** https://www.loom.com/share/1b41d38270a547a280ba8cabcc850d8c
- **Local copy:** [assets/Music Discovery Engine Demo.mp4](<assets/Music Discovery Engine Demo.mp4>)

The video covers the four rubric requirements: end-to-end runs with three different inputs, AI feature behavior (the RAG explanation generated by Gemini for each result), reliability behavior (the per-result confidence badge, plus the Discovery mode toggle as an echo-chamber guardrail), and clear named outputs at each step.
